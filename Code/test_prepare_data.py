    ...
    for x in range(0, len(fill_data_1)):
        with open(files_1[i], 'r') as f:
            ...
            for row in reader:
                # Skip the header row.
                if rowNr >= 1:
                    if int(row[0]) in fill_data_1[x]:
                        print("Already here")
                    else:
                        fill_data_1[x][int(row[0])] = row[1]
    ...
    for z in range(0, len(fill_data_2)):
        with open(files_2[j], 'r') as f:
            ...
            for row in reader:
                # Skip the header row.
                if rowNr >= 1:
                    if int(row[0]) not in fill_data_2[z]:
                        fill_data_2[z][int(row[0])] = {}
                        fill_data_2[z][int(row[0])][int(row[1])] = (row[2])
                    else:
                        fill_data_2[z][int(row[0])][int(row[1])] = (row[2])
                # Increase the row number
                rowNr = rowNr + 1
